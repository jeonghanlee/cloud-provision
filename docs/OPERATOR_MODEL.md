...
| P_mariadb | `mariadb` | After P_common | A MariaDB server on loopback with the `archappl` database and application user, the Archiver Appliance config store. It listens on the IPv4 loopback address only (127.0.0.1:3306), not on IPv6 (`::1`), and the application account authenticates over that TCP loopback connection with a password that must match the credential aa-env's Tomcat JNDI resource carries, not one this operator invents. To keep that login deterministic regardless of how the host resolves `localhost` (which can map to `::1`), the server runs `skip-name-resolve` and the account takes the IP form `@'127.0.0.1'`, and aa-env connects to `127.0.0.1`; root stays socket-authenticated (unaffected by `skip-name-resolve`) and nothing listens beyond loopback. The schema, the Tomcat JNDI resource, and the `make db.*` application stay aa-env's. Kept a separable module: aa-env replaces MariaDB with SQLite in its Phase 2, so this operator drops out then. Used by the archiver species only. |
...
| P_archiver-build | `archiver_build` | After P_java, P_tomcat, P_mariadb | Builds the Archiver Appliance WARs from the aa-maven source at its freeze tag on the base VM with the repository `./mvnw` wrapper, then installs as P_archiver does. Needs git (with an origin ref) in addition to P_java; Maven dependency resolution reaches its repositories over HTTPS on a cold `~/.m2` cache, which a warm cache or `-o` avoids. No asset is fetched during the build - the svg_viewer asset is vendored in-tree. The source-build alternative to P_archiver; never both. |
