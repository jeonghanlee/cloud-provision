...
| | M1.4 | [Preserve pinned golden images across rebakes (#2)](https://github.com/jeonghanlee/cloud-provision/issues/2) | ⬜ | ▶ | | Rebakes use new filenames, pinned images remain until downstream pins advance, and the retention rule is documented. Design accepted 2026-07-31 in `docs/DESIGN_IMAGE_LAYOUT.md`, shared with M1.6: archive entries are named from the `bake_date` the manifest already records, and the current plus previous entry are retained per platform. Implementation is not authorized by that design. |
